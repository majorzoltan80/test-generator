...
Graph.Layout.Symmetric = function(graph) {
    this.graph = graph;
	this.layout();
};
Graph.Layout.Symmetric.prototype = {
    layout: function() {
        this.layoutPrepare();
        this.layoutCalcBounds();
    },
    ...
    layoutPrepare: function() {
       	//using the Roots of unity to put them into a circle
		var n=0;
		for (i in this.graph.nodes){
			n++;
		}
		...
		for (i in this.graph.nodes) {
            var node = this.graph.nodes[i];
            node.layoutPosX = Math.cos(2*Math.PI*counter/n);
			node.layoutPosY = Math.sin(2*Math.PI*counter/n);
			counter++;
        }
    },
